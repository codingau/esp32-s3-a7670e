...
static void _usbh_processing_task(void* arg) {
    ...
    ESP_LOGI(TAG, "USB Processing Start 001");


    while (1) {
        /* handle usb hotplug */
        ESP_LOGI(TAG, "Waiting USB Connection 002");
        ...
        while (!(xEventGroupGetBits(evt_group_hdl) & USBH_TASK_KILL_BIT)) {
            /* handle usb event */
            if (xEventGroupGetBits(evt_group_hdl) & USBH_TASK_SUSPEND_BIT) {
                vTaskDelay(pdMS_TO_TICKS(100));
                continue;
            }
            if (xQueueReceive(usb_queue_hdl, &evt_msg, 1) != pdTRUE) {
                vTaskDelay(pdMS_TO_TICKS(100));
                continue;
            }
